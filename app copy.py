...
# Dados de configuração da planilha
...
for i in range(num_amostras):
    ...
    # Botão de adicionar
    if st.button(f"Adicionar Amostra {i+1}", key=f"add_{i}"):
        try:
...
if st.session_state.dados:
    dados_para_salvar = pd.DataFrame(st.session_state.dados)
    dados_para_salvar["Data"] = dados_para_salvar["Data"].astype(str)  # Converte datetime para string
